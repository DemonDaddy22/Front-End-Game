...
const handleStartGame = () => {
  ...
  blocksSequence.forEach((blockId, index) => {
    ...
    setTimeout(() => { block.classList.add('background-correct'); },
               500 + 1000 * index);
    setTimeout(() => {
...
const handleBlockClick = (e) => {
  if (showingSequence || !blocksSequence.length)
    return;
  ...
  if (block.dataset.id == blocksSequence[gameCounter]) {
    ...
    if (gameCounter === blocksSequence.length) {
      ...
      highScore.textContent = hScore;
      blocksSequence = [];
      localStorage.setItem('memoryGameHighScore', JSON.stringify(hScore));
    }
  } else {
