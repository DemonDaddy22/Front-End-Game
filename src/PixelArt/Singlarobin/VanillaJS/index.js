...
function PixelArt(el, rows, cols) {
  ...
  const cellWidthString = getComputedStyle(document.documentElement)
                              .getPropertyValue('--cell-width');

  const cellWidthInt =
      parseInt(cellWidthString.substring(1, cellWidthString.indexOf('p')));
...
function getColorValue(list) {
  ...
  while (color.length !== 6 || color === '011d49' || color === '808080' ||
         list.includes(color)) {
...
function handleMouseEvent() { draggable = !draggable; }
